# Support

## Getting Help

Use the official Dot. documentation first:

```text
https://dot.mindreset.tech/docs/service/open/skill
```

For security-sensitive issues, follow the official responsible disclosure policy and email
`security@mindreset.tech`:

```text
https://dot.mindreset.tech/docs/security_policy
```

For repository issues, feature requests, or documentation fixes, open an issue:

```text
https://github.com/MindReset/dot_skill/issues
```

## Before Opening an Issue

Please include:

- The helper script or platform you are using, such as Codex, `npx skills add`, or direct Python
- Your operating system and Python version
- The command you ran, with API keys and device IDs removed
- The HTTP status code and error response, if any
- Whether the matching Text API, Image API, or Canvas API content item already exists in the Dot. App Content Studio loop task

## Sensitive Data

Never paste:

- `DOT_API_KEY` values
- Full device serial numbers
- Private image URLs
- Personal content rendered on your Dot. device

## Scope

This repository covers the Dot Skill files, helper scripts, OpenAPI schema, and Codex plugin
metadata. For account, billing, device hardware, or Dot. App issues, use the official Dot. support
channels from:

```text
https://dot.mindreset.tech
```
